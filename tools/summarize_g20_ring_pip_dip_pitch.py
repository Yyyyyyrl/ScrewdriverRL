#!/usr/bin/env python3
"""Summarize the fixed-D435 G20 ring PIP/DIP/MCP-pitch calibration."""
...
def pip_camera_path(session: Path, raw: int) -> Path:
    suffix = (
        "camera_fixed_exp_pca_split_180f_summary.json"
        if raw <= 20
        else "camera_fixed_exp_180f_summary.json"
    )
    return session / f"pip_forward/flex_raw{raw:03d}/{suffix}"


def pitch_camera_path(session: Path, raw: int) -> Path:
    if raw <= 48:
        return session / (
            f"pitch_forward_after_manual_clearance/flex_raw{raw:03d}/"
            "camera_four_real_markers_fixed_exp_180f_summary.json"
        )
    return session / f"pitch_forward/flex_raw{raw:03d}/camera_fixed_exp_180f_summary.json"
...
def pip_motion(session: Path, raw: int) -> dict[str, Any]:
    return sent_motion(session / f"pip_forward/flex_raw{raw:03d}")


def pitch_motion(session: Path, raw: int) -> dict[str, Any]:
    directory = (
        session / f"pitch_forward_after_manual_clearance/flex_raw{raw:03d}"
        if raw <= 48
        else session / f"pitch_forward/flex_raw{raw:03d}"
    )
...
def scan_motion_health(session: Path) -> dict[str, Any]:
    ...
    for path in session.rglob("*.json"):
        ...
        if "ring" not in operation and not scope_joint.startswith("ring"):
            continue
        files.append(path)
        ...
        for row in payload["post_command_samples"]:
            ...
            if any(faults[slot] for slot in (3, 8, 18)):
                ring_fault_samples += 1
            max_ring_temp = max(max_ring_temp, *(temps[slot] for slot in (3, 8, 18)))
    return {
        "sent_ring_motion_files": len(files),
        "sent_ring_motion_files_without_post_samples": files_without_post_samples,
        "recorded_post_command_samples": samples,
        "ring_fault_samples": ring_fault_samples,
        "max_ring_temperature_c": max_ring_temp,
    }
...
def main() -> int:
    ...
    pip_zero_paths = [
        session / "pip_raw255_zero/camera_fixed_exp_180f_a_summary.json",
        session / "pip_raw255_zero/camera_fixed_exp_180f_b_summary.json",
    ]
    pitch_zero_paths = [
        session / "pitch_zero_after_manual_clearance/camera_four_real_markers_fixed_exp_180f_a_summary.json",
        session / "pitch_zero_after_manual_clearance/camera_four_real_markers_fixed_exp_180f_b_summary.json",
    ]
    pip_zero, pip_repeat = mean_angles(pip_zero_paths)
    pitch_zero, pitch_repeat = mean_angles(pitch_zero_paths)

    final_snapshot_path = session / "final_sdk_snapshot_after_manual_clearance.json"
    final_snapshot = read_json(final_snapshot_path)
    final_rows = final_snapshot["samples"]
    final_pip_readback = statistics.median(row["state20"][18] for row in final_rows)
    final_pitch_readback = statistics.median(row["state20"][3] for row in final_rows)
    ...
    for raw in RAWS:
        if raw == 255:
            ...
            pip_readback = stable_state(pip_payload, 18)
            pitch_readback = stable_state(pitch_payload, 3)
            pip_quality = "formal_180f_pca_split" if raw <= 20 else "formal_180f"
            pitch_quality = "formal_180f_four_real_markers_no_split" if raw <= 48 else "formal_180f"
        ...
        point = {
            ...
            "ring_pip_rad": math.radians(pip_zero["pip_projected_deg_2d"] - pip_angles["pip_projected_deg_2d"]),
            "ring_dip_rad": math.radians(pip_zero["dip_projected_deg_2d"] - pip_angles["dip_projected_deg_2d"]),
            "ring_mcp_pitch_rad": math.radians(pitch_zero["mcp_projected_deg_2d"] - pitch_angles["mcp_projected_deg_2d"]),
    ...
    for key in ("ring_pip_rad", "ring_dip_rad", "ring_mcp_pitch_rad"):
    ...
    pip_return_path = session / "pip_return/camera_raw128_return_fixed_exp_180f_summary.json"
    pitch_forward_path = pitch_camera_path(session, 128)
    pitch_return_path = session / "pitch_return/camera_raw128_return_fixed_exp_180f_summary.json"
    ...
    pip_rad = [point["ring_pip_rad"] for point in points]
    dip_rad = [point["ring_dip_rad"] for point in points]
    pitch_rad = [point["ring_mcp_pitch_rad"] for point in points]
    ...
    summary = {
        ...
        "camera_contract": {
            ...
            "separate_zero_pairs_for_pip_and_pitch_phases": True,
            "dynamic_other_finger_occlusion_schedule": True,
            "ring_and_camera_fixed_across_occluder_reposition": True,
        ...
        "points": points,
        "ranges": {
            "ring_pip_max_rad": pip_rad[0],
            "ring_pip_max_deg": math.degrees(pip_rad[0]),
            "ring_pip_raw0_stable_readback_raw": points[0]["pip_stable_readback_raw"],
            "ring_dip_max_rad": dip_rad[0],
            "ring_dip_max_deg": math.degrees(dip_rad[0]),
            "ring_mcp_pitch_max_rad": pitch_rad[0],
            "ring_mcp_pitch_max_deg": math.degrees(pitch_rad[0]),
        },
        "mimic_fit": fit_mimic(pip_rad, dip_rad),
        "hysteresis_raw128": {
            "ring_pip_rad": math.radians(pip_forward["pip_projected_deg_2d"] - pip_return["pip_projected_deg_2d"]),
            "ring_pip_deg": pip_forward["pip_projected_deg_2d"] - pip_return["pip_projected_deg_2d"],
            "ring_dip_rad": math.radians(pip_forward["dip_projected_deg_2d"] - pip_return["dip_projected_deg_2d"]),
            "ring_dip_deg": pip_forward["dip_projected_deg_2d"] - pip_return["dip_projected_deg_2d"],
            "ring_mcp_pitch_rad": math.radians(pitch_forward["mcp_projected_deg_2d"] - pitch_return["mcp_projected_deg_2d"]),
            "ring_mcp_pitch_deg": pitch_forward["mcp_projected_deg_2d"] - pitch_return["mcp_projected_deg_2d"],
        ...
        "heldout_piecewise_linear_max_abs_error_rad": {
            "ring_pip": heldout_error(pip_rad),
            "ring_dip": heldout_error(dip_rad),
            "ring_mcp_pitch": heldout_error(pitch_rad),
        },
        ...
        "final_sdk_snapshot": {
            ...
            "ring_pip_readback_median_raw": final_pip_readback,
            "ring_pitch_readback_median_raw": final_pitch_readback,
        },
        "limitations": [
            "PIP raw20 and below used validated PCA splitting for merged blue-tape components; all formal captures completed 180/180.",
            "Pitch raw48 and below uses 180/180 captures with four real marker components and merged-marker splitting explicitly disabled.",
            "The first deep-pitch batch is retained as rejected evidence because the root marker was occluded and a synthetic split produced a non-monotonic curve.",
            "Other fingers were repositioned between deep-flex and raw255 zero captures; the ring and fixed camera were not moved, and all accepted captures show four real ring markers.",
            "Ring side readback showed up to 3 raw of pitch-coupled slack while commands held side at 128; this is recorded and not treated as perfect tracking.",
            "Candidate calibration is not production-enabled; pinky PIP/DIP/pitch remains uncalibrated.",
        ],
        "input_sha256": {
    ...
    write_json(session / "ring_pip_dip_pitch_summary.json", summary)

    csv_path = session / "ring_pip_dip_pitch_calibration_points.csv"
    ...
    candidate["note"] = (
        ...
        "PIP/MCP pitch plus thumb MCP/CMC pitch; pinky incomplete"
    )
    candidate["joints"]["ring_pip"] = {
        "flip": False,
        "lo": 0.0,
        "hi": pip_rad[0],
        "physical_lut": {"raw": list(RAWS), "rad": pip_rad},
    }
    candidate["joints"]["ring_mcp_pitch"] = {
        "flip": False,
        "lo": 0.0,
        "hi": pitch_rad[0],
    ...
    report = f"""# Ring PIP/DIP/MCP-pitch fixed-exposure calibration

Status: complete as a candidate calibration; not production-enabled.

## Result

- Ring PIP command raw0 settled at raw `{points[0]['pip_stable_readback_raw']:.0f}` and measured `{pip_rad[0]:.10f} rad / {math.degrees(pip_rad[0]):.3f} deg`.
- Passive ring DIP measured `{dip_rad[0]:.10f} rad / {math.degrees(dip_rad[0]):.3f} deg` at that endpoint.
- Ring MCP pitch raw0 measured `{pitch_rad[0]:.10f} rad / {math.degrees(pitch_rad[0]):.3f} deg`; raw0 readback was `{points[0]['pitch_stable_readback_raw']:.0f}`.
...
- Raw128 same-camera hysteresis: PIP `{hyst['ring_pip_rad']:+.10f} rad / {hyst['ring_pip_deg']:+.3f} deg`, DIP `{hyst['ring_dip_rad']:+.10f} rad / {hyst['ring_dip_deg']:+.3f} deg`, pitch `{hyst['ring_mcp_pitch_rad']:+.10f} rad / {hyst['ring_mcp_pitch_deg']:+.3f} deg`.
- Held-out piecewise-linear maximum errors: PIP `{heldout['ring_pip']:.10f} rad`, DIP `{heldout['ring_dip']:.10f} rad`, pitch `{heldout['ring_mcp_pitch']:.10f} rad`.

## Camera and measurement contract

PIP/DIP and MCP-pitch LUT points use the same fixed side-camera pose. Each phase has its own pair of 180-frame raw255 zero references so time-separated drift is not mixed into the LUT. All physical angles are changes from the matching phase zero. Other fingers used different clearance poses for deep flex and raw255, but the ring and fixed camera were not moved; every accepted pitch frame contained four real ring marker components.
...
Every accepted formal PIP and pitch point completed 180/180 usable frames. PIP raw20 and below used the validated PCA split for true ring self-merges. The first deep-pitch batch was rejected after live inspection showed that another finger occluded the root marker. The replacement raw48-and-below batch explicitly disabled splitting and accepted only four real marker components; its curve passed strict monotonicity.
...
- Recorded ring-motion files: `{health['sent_ring_motion_files']}`; post-command samples: `{health['recorded_post_command_samples']}`; ring fault samples: `{health['ring_fault_samples']}`; maximum ring-slot temperature: `{health['max_ring_temperature_c']} C`.
...
- Final raw255 visual repeat completed 180/180, and the final SDK snapshot completed 20/20 state=ok with no active fault.
- Pitch motion showed up to 3 raw of side-axis coupled slack while every command held ring side at 128; target pitch and held PIP remained within 2 raw.

## Software implication

Post-generation validation for this recorded candidate: both ring LUTs round-tripped all 19 measured knots with zero raw error and zero radian error. The Topdown live gate recognizes the ring LUTs and still refuses deployment because pinky pitch/PIP remain uncalibrated. Targeted regression: `42 passed`.

The measured ring PIP physical endpoint is above both the old candidate `1.08 rad` and the current generic `1.57 rad` assumption. The measured ring pitch endpoint is `{pitch_rad[0]:.10f} rad`. The new JSON is deliberately candidate-only: production calibration, semantic limits, URDF mimic multiplier and policy deployment remain unchanged until pinky is measured and the full-hand gate is reviewed.

Artifacts:

- `ring_pip_dip_pitch_summary.json`
- `ring_pip_dip_pitch_calibration_points.csv`
- `{args.candidate_out.resolve()}`
- `final_sdk_snapshot_after_manual_clearance.json`
"""
    (session / "RING_PIP_DIP_PITCH_FIXED_EXPOSURE_CALIBRATION_REPORT.md").write_text(
        report, encoding="utf-8"
    )
    print(json.dumps({
        "summary": str(session / "ring_pip_dip_pitch_summary.json"),
        "points": str(csv_path),
        "report": str(session / "RING_PIP_DIP_PITCH_FIXED_EXPOSURE_CALIBRATION_REPORT.md"),
