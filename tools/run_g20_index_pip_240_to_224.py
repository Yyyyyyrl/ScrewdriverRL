#!/usr/bin/env python3
"""Run the single approved G20 index-PIP calibration step: raw 240 -> 224.

This tool is intentionally narrow and fail-closed:

* it accepts only the left G20 hand with the expected serial number;
* it requires the index PIP state (slot 16) to start at raw 240;
...
* it does not continue to raw 208 and does not automatically return to 240.
...
def main() -> int:
    ...
    payload: dict[str, Any] = {
        "schema_version": 1,
        "operation": "g20_index_pip_step_raw240_to_raw224",
        "approved_scope": {
            "joint": "index_pip",
            "raw20_slot": INDEX_PIP_SLOT,
            "start_raw": EXPECTED_START_RAW,
            "target_raw": ONLY_ALLOWED_TARGET_RAW,
            "continue_to_raw208": False,
            "automatic_return_to_raw240": False,
        },
        "transport": {
            "side": "left",
            "hand_joint": "G20",
    ...
    try:
        ...
        print(f"[g20-240-to-224] raw20 before: {before20}", flush=True)
        print(f"[g20-240-to-224] raw20 after:  {after20}", flush=True)
        print(f"[g20-240-to-224] only diff:    {raw20_diff}", flush=True)
        print(f"[g20-240-to-224] index 0x42:   {index_command6}", flush=True)
        ...
        for sample_index in range(args.samples):
            ...
            print(
                f"[g20-240-to-224] sample={sample_index:02d} "
        ...
        _write_json(args.out, payload)
        print(f"[g20-240-to-224] wrote {args.out}", flush=True)
        return 0
