...
def main() -> None:
    ...
    # Keep the monotonic forward sweep and one zero-reference image.  The return
    # and duplicate-zero samples remain in the full-fit metrics reported below.
    rows = [row for row in fit["rows"] if row["direction"] == "forward"]
    rows.sort(key=lambda row: row["stable_readback_raw"], reverse=True)
