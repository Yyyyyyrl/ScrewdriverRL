#!/usr/bin/env python3
"""Run one adjacent, isolated G20 ring PIP or MCP-pitch calibration step.

The vendor G20 ring 0x44 frame is
``[side(s8), reserved, pitch(s3), reserved, reserved, pip(s18)]``.
The vendor raw20 adapter historically aliases the fifth ring frame byte to
slot15 even though ring state readback treats it as reserved. This runner
therefore validates the three reserved bytes directly from fresh ``x44`` and
...
sends exactly one ring-finger frame, and then records state/fault/temperature.
...
RING_SIDE_SLOT = 8
RING_PIP_SLOT = 18
# The fifth ring frame byte has no trustworthy raw20 identity. The vendor
# command mapper aliases it to slot15, while state readback treats it reserved.
RING_FRAME_RAW20_SLOTS = (8, 13, 3, 14, None, 18)
APPROVED_RING_TORQUE6 = [40, 20, 60, 20, 20, 40]
JOINT_TO_SLOT = {
    "ring_pip": RING_PIP_SLOT,
    "ring_mcp_pitch": RING_PITCH_SLOT,
}


def _fresh_ring_state6(api: Any) -> list[int]:
    api.hand.get_ring_positions()
    time.sleep(0.04)
    return _int_vector(api.hand.x44, length=6, label="ring_state6")


def build_ring_command(
    state20: Sequence[int],
    ring_state6: Sequence[int],
    *,
    joint: str,
    ...
    """Validate raw20/0x44 identity and build one isolated ring command."""
    ...
    ring_before = _int_vector(
        ring_state6, length=6, label="ring_state6"
    )
    for index, slot in enumerate(RING_FRAME_RAW20_SLOTS):
        actual = ring_before[index]
        if slot is None:
            if actual != 0:
                raise ValueError(
                    "fresh ring frame reserved byte 4 must be zero, "
                    f"got {actual}"
                )
            continue
        ...
        if abs(actual - expected) > tolerance:
            raise ValueError(
                "fresh ring frame disagrees with raw20: "
    ...
    ring_after = list(ring_before)
    ring_after[0] = side_hold_raw
    ring_after[2] = (
        target_raw
        if target_slot == RING_PITCH_SLOT
    ...
    if ring_after[1:2] + ring_after[3:5] != [0, 0, 0]:
        raise AssertionError(f"unsafe ring reserved values: {ring_after}")
    return after, ring_after, diff
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "safety_contract": {
            "position_method": "api.hand.set_ring_positions",
            "position_can_frame": "0x44",
            "frame_raw20_slots": list(RING_FRAME_RAW20_SLOTS),
            "finger_move_called": False,
            "other_finger_position_frames_sent": False,
            "ring_speed_only": args.speed,
            "ring_torque6": APPROVED_RING_TORQUE6,
    ...
    try:
        ...
        ring_before6 = _fresh_ring_state6(api)
        after20, ring_command6, raw20_diff = build_ring_command(
            before20,
            ring_before6,
            joint=args.joint,
        ...
        payload["command"] = {
            ...
            "ring_frame6_before": ring_before6,
            "ring_frame6_sent": ring_command6,
        }
        _write_json(args.out, payload)
        print(f"[g20-ring-step] raw20 before: {before20}", flush=True)
        print(f"[g20-ring-step] only diff:    {raw20_diff}", flush=True)
        print(f"[g20-ring-step] ring 0x44: {ring_command6}", flush=True)

        api.hand.set_ring_speed([args.speed] * 6)
        api.hand.set_ring_torque(APPROVED_RING_TORQUE6)
        time.sleep(0.05)
        payload["motion_command_wall_time_s"] = time.time()
        api.hand.set_ring_positions(ring_command6)
        ...
        for sample_index in range(args.samples):
            ...
            print(
                f"[g20-ring-step] sample={sample_index:02d} "
                f"target={state20[target_slot]} "
                f"side={state20[RING_SIDE_SLOT]} "
            ...
            if any(sample_faults20):
                ...
                for slot, hold in (
                    (RING_SIDE_SLOT, args.side_hold_raw),
        ...
        excluded = {target_slot, RING_SIDE_SLOT, other_slot}
        target_error = settled_median[target_slot] - args.target_raw
        side_error = settled_median[RING_SIDE_SLOT] - args.side_hold_raw
        other_flex_error = (
        ...
        _write_json(args.out, payload)
        print(f"[g20-ring-step] wrote {args.out}", flush=True)
        if not settled:
            print(
                "[g20-ring-step] NOT SETTLED; refusing automatic continuation",
                flush=True,
            )
            return 2
        return 0
