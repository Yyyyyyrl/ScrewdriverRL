#!/usr/bin/env python3
"""Run one adjacent, isolated G20 thumb CMC-yaw calibration step.

The vendor G20 thumb 0x41 frame is
``[roll(s5), yaw(s10), pitch(s0), r11, r12, mcp(s15)]``. Yaw is element 1.

Unlike the Phase B long-finger roll runner, every non-target thumb axis lives in
this same frame, so roll, pitch and MCP are actively commanded to their
isolation holds rather than only verified. Isolation here is therefore stronger
than it was for the four-finger rolls.

Yaw has no raw255 home: like roll it is bidirectional with a mid-range
...
def build_thumb_yaw_command(
    ...
    *,
    ...
    """Validate raw20/0x41 identity and build one isolated yaw command."""
    ...
    for index, slot, tolerance in (
        (0, THUMB_ROLL_SLOT, hold_tolerance_raw),
        (FRAME_YAW_INDEX, THUMB_YAW_SLOT, start_tolerance_raw),
    ...
    if abs(before[THUMB_YAW_SLOT] - expected_start_raw) > start_tolerance_raw:
        raise ValueError(
            f"slot {THUMB_YAW_SLOT} must start within {start_tolerance_raw} raw "
            f"of {expected_start_raw}, got {before[THUMB_YAW_SLOT]}")
    actual_start_raw = before[THUMB_YAW_SLOT]
    step_raw = abs(target_raw - actual_start_raw)
    if not 1 <= step_raw <= MAX_ADJACENT_STEP_RAW:
        raise ValueError(
            f"actual thumb yaw step must be 1..{MAX_ADJACENT_STEP_RAW} raw, got "
    ...
    after = list(before)
    after[THUMB_YAW_SLOT] = target_raw
    for slot, hold in holds.items():
        after[slot] = hold
    frame_after = list(frame_before)
    frame_after[FRAME_YAW_INDEX] = target_raw
    frame_after[0] = roll_hold_raw
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {
        "schema_version": 1,
        "operation": (f"g20_thumb_cmc_yaw_candidate_step_raw"
    ...
    try:
        ...
        after20, frame_command6, raw20_diff = build_thumb_yaw_command(
        ...
        print(f"[g20-thumb-yaw] raw20 before: {before20}", flush=True)
        print(f"[g20-thumb-yaw] only diff:    {raw20_diff}", flush=True)
        print(f"[g20-thumb-yaw] 0x41:         {frame_command6}", flush=True)
        if not args.execute:
            payload["refused_reason"] = "--execute not supplied"
            payload["ended_wall_time_s"] = time.time()
            _write_json(args.out, payload)
            print("[g20-thumb-yaw] preflight only; no frame sent", flush=True)
            return 0
        ...
        period = 1.0 / args.hz
        holds = {THUMB_ROLL_SLOT: args.roll_hold_raw,
        ...
        for sample_index in range(args.samples):
            ...
            print(f"[g20-thumb-yaw] sample={sample_index:02d} "
                  ...
                  f"temp={temperature20[THUMB_YAW_SLOT]}", flush=True)
            stop_reason = None
            if any(sample_faults20):
                stop_reason = "nonzero fault after command"
            elif (temperature20[THUMB_YAW_SLOT]
                  - temperature_before20[THUMB_YAW_SLOT] >= 8):
        ...
        target_error = settled_median[THUMB_YAW_SLOT] - args.target_raw
        fault_free = not any(v for r in records for v in r["faults20"])
        excluded = {THUMB_YAW_SLOT, *holds}
        ...
        payload["ended_wall_time_s"] = time.time()
        _write_json(args.out, payload)
        print(f"[g20-thumb-yaw] wrote {args.out}", flush=True)
        if not settled:
            print("[g20-thumb-yaw] NOT SETTLED; refusing automatic continuation",
                  flush=True)
            return 2
        return 0
