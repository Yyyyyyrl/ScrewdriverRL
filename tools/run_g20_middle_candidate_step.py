#!/usr/bin/env python3
"""Run one adjacent, isolated G20 middle PIP or MCP-pitch calibration step.

The vendor G20 middle 0x43 frame is
``[side(s7), r12, pitch(s2), r13, r14, pip(s17)]``.  This tool holds the
other flexion axis at the session isolation reference, changes one target,
sends exactly one middle-finger frame, and then records state/fault/temperature.
...
MIDDLE_SIDE_SLOT = 7
MIDDLE_PIP_SLOT = 17
MIDDLE_FRAME_SLOTS = (7, 12, 2, 13, 14, 17)
APPROVED_MIDDLE_TORQUE6 = [40, 20, 60, 20, 20, 40]
JOINT_TO_SLOT = {
    "middle_pip": MIDDLE_PIP_SLOT,
    "middle_mcp_pitch": MIDDLE_PITCH_SLOT,
}


def _fresh_middle_state6(api: Any) -> list[int]:
    api.hand.get_middle_positions()
    time.sleep(0.04)
    return _int_vector(api.hand.x43, length=6, label="middle_state6")


def build_middle_command(
    state20: Sequence[int],
    middle_state6: Sequence[int],
    *,
    joint: str,
    ...
    """Validate raw20/0x43 identity and build one isolated middle command."""
    ...
    middle_before = _int_vector(
        middle_state6, length=6, label="middle_state6"
    )
    expected_frame = [before[slot] for slot in MIDDLE_FRAME_SLOTS]
    for index, (actual, expected) in enumerate(
        zip(middle_before, expected_frame)
    ):
        tolerance = 0 if index in (1, 3, 4) else hold_tolerance_raw
        if abs(actual - expected) > tolerance:
            raise ValueError(
                "fresh middle frame disagrees with raw20: "
    ...
    other_slot = (
        MIDDLE_PITCH_SLOT
        if target_slot == MIDDLE_PIP_SLOT
        else MIDDLE_PIP_SLOT
    )
    ...
    holds = {
        MIDDLE_SIDE_SLOT: side_hold_raw,
    ...
    middle_after = [after[slot] for slot in MIDDLE_FRAME_SLOTS]
    diff = [
    ...
    if middle_after[1:2] + middle_after[3:5] != [0, 0, 0]:
        raise AssertionError(f"unsafe middle reserved values: {middle_after}")
    return after, middle_after, diff
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "safety_contract": {
            "position_method": "api.hand.set_middle_positions",
            "position_can_frame": "0x43",
            "frame_slots": list(MIDDLE_FRAME_SLOTS),
            "finger_move_called": False,
            "other_finger_position_frames_sent": False,
            "middle_speed_only": args.speed,
            "middle_torque6": APPROVED_MIDDLE_TORQUE6,
    ...
    try:
        ...
        middle_before6 = _fresh_middle_state6(api)
        after20, middle_command6, raw20_diff = build_middle_command(
            before20,
            middle_before6,
            joint=args.joint,
        ...
        payload["command"] = {
            ...
            "middle_frame6_before": middle_before6,
            "middle_frame6_sent": middle_command6,
        }
        _write_json(args.out, payload)
        print(f"[g20-middle-step] raw20 before: {before20}", flush=True)
        print(f"[g20-middle-step] only diff:    {raw20_diff}", flush=True)
        print(f"[g20-middle-step] middle 0x43: {middle_command6}", flush=True)

        api.hand.set_middle_speed([args.speed] * 6)
        api.hand.set_middle_torque(APPROVED_MIDDLE_TORQUE6)
        time.sleep(0.05)
        payload["motion_command_wall_time_s"] = time.time()
        api.hand.set_middle_positions(middle_command6)
        ...
        for sample_index in range(args.samples):
            ...
            print(
                f"[g20-middle-step] sample={sample_index:02d} "
                f"target={state20[target_slot]} "
                f"side={state20[MIDDLE_SIDE_SLOT]} "
            ...
            if any(sample_faults20):
                ...
                for slot, hold in (
                    (MIDDLE_SIDE_SLOT, args.side_hold_raw),
        ...
        excluded = {target_slot, MIDDLE_SIDE_SLOT, other_slot}
        target_error = settled_median[target_slot] - args.target_raw
        side_error = settled_median[MIDDLE_SIDE_SLOT] - args.side_hold_raw
        other_flex_error = (
        ...
        _write_json(args.out, payload)
        print(f"[g20-middle-step] wrote {args.out}", flush=True)
        if not settled:
            print(
                "[g20-middle-step] NOT SETTLED; refusing automatic continuation",
                flush=True,
            )
            return 2
        return 0
