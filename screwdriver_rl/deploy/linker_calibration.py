...
This is the architecture M0 source of truth.  It deliberately has no module-
...
DEFAULT_SCHEMA_PATH = (
    _REPO_ROOT / "assets" / "calibrations" / "linker_g20_left_semantic_schema_v1.json"
)
DEFAULT_CALIBRATION_PATH = (
    _REPO_ROOT / "assets" / "calibrations" / "linker_g20_left_lht20_010_415_v1.json"
)
...
def load_linker_calibration(
    ...
    if mapping_mode not in (
        "normalized-semantic-fraction-to-native-arc",
        "piecewise-linear-raw-to-semantic",
    ):
    ...
    for spec, entry in zip(schema.joints, entries):
        ...
        if mapping_mode == "piecewise-linear-raw-to-semantic":
            if not isinstance(lut, dict) or set(lut) != {"raw", "rad"}:
                raise ValueError(f"{spec.name}: physical LUT must contain raw and rad")
            raw_values = lut["raw"]
            rad_values = lut["rad"]
            if (
                not isinstance(raw_values, list)
                or not isinstance(rad_values, list)
                or len(raw_values) != len(rad_values)
                or len(raw_values) < 2
            ):
                raise ValueError(f"{spec.name}: physical LUT vectors are invalid")
            raw_knots = tuple(float(value) for value in raw_values)
            rad_knots = tuple(float(value) for value in rad_values)
            if any(not math.isfinite(value) for value in (*raw_knots, *rad_knots)):
                raise ValueError(f"{spec.name}: physical LUT contains non-finite values")
            if (
                raw_knots[0] < 0.0
                or raw_knots[-1] > 255.0
                or any(a >= b for a, b in zip(raw_knots, raw_knots[1:]))
            ):
                raise ValueError(f"{spec.name}: physical raw knots must increase in 0..255")
            rad_increasing = all(a < b for a, b in zip(rad_knots, rad_knots[1:]))
            rad_decreasing = all(a > b for a, b in zip(rad_knots, rad_knots[1:]))
            if not (rad_increasing or rad_decreasing):
                raise ValueError(f"{spec.name}: physical rad knots must be monotonic")
            if min(rad_knots) > lower or max(rad_knots) < upper:
                raise ValueError(f"{spec.name}: physical LUT does not span soft limit")
            if flip:
                raise ValueError(f"{spec.name}: physical LUT cannot be combined with flip")
        elif lut is not None:
            raise ValueError(f"{spec.name}: physical LUT requires piecewise mapping mode")
        joints.append(
            JointCalibration(
                spec.name,
                slot,
...
def _interp_clamped(
    value: float, xs: Sequence[float], ys: Sequence[float]
) -> float:
    if value <= xs[0]:
        return float(ys[0])
    if value >= xs[-1]:
...
def _semantic_to_raw(value: float, joint: JointCalibration) -> float:
    raw = joint.physical_raw_knots
    rad = joint.physical_rad_knots
    if raw is None or rad is None:
        raise ValueError(f"{joint.name}: no physical LUT")
    if rad[0] < rad[-1]:
        return _interp_clamped(value, rad, raw)
    return _interp_clamped(value, tuple(reversed(rad)), tuple(reversed(raw)))
...
class LinkerMapper:
    ...
    def preview(self, semantic: Sequence[float]) -> MappingPreview:
        ...
        for value, joint in zip(incoming, joints):
            ...
            if joint.physical_raw_knots is not None:
                native_raw = _semantic_to_raw(adjusted, joint)
                fraction = native_raw / 255.0
    ...
    def inverse(self, native_range: Sequence[float]) -> tuple[float, ...]:
        ...
        for joint in self.calibration.joints:
            ...
            if joint.physical_raw_knots is not None:
                semantic = _interp_clamped(
                    value, joint.physical_raw_knots, joint.physical_rad_knots
                ) - joint.offset
            else:
