...
HEIGHT_TARGETS = {"middle": 0.70, "ring": 0.60, "pinky": 0.50, "thumb": 0.60}
...
def main() -> None:
    ...
    for i in range(STARTS):
        ...
        band_ok = all(0.20 <= hf[f] <= 0.90 for f in ("middle", "ring", "pinky", "thumb"))
        opp = abs(hf["thumb"] - np.mean([hf[f] for f in ("middle", "ring", "pinky")]))
        opp_ok = opp * (obj.body_top_z - obj.body_base_z) <= 0.020
    ...
    with open(SCRATCH + "fit_wrench2.json", "w") as f:
